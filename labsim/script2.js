...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function showTab(targetId) {
        tabContents.forEach(content => {
            if (content.id === targetId) {
                ...
                setTimeout(() => {
                    content.style.transition = "opacity 0.5s, transform 0.5s cubic-bezier(.68,-0.55,.27,1.55)";
    ...
    if (tabContents.length > 0) {
        tabContents.forEach((c, i) => {
            ...
            c.style.transform = i === 0 ? 'translateY(0) scale(1)' : 'translateY(40px) scale(0.96)';
            c.style.transition = "opacity 0.5s, transform 0.5s cubic-bezier(.68,-0.55,.27,1.55)";
        });
    }
